...
inline static uint16_t rgb16(uint8_t r, uint8_t g, uint8_t b)
{
    return ((((((uint16_t)r) >> 3) & 31) << 11) |
           (((((uint16_t)g) >> 2) & 63) << 5)  |
           ((((uint16_t)b) >> 3) & 31) );
}
...
int main() 
{
    ...
    LOGI("xoffset: %d, yoffset: %d, line_len: %d\n", vinfo.xoffset, 
    ...
    fbp = (uint8_t *)mmap(0, screensize, PROT_READ | PROT_WRITE, MAP_SHARED,
                       fbfd, 0);
        ...
        for ( y = (vinfo.yres - guage_height) / 2; y < (vinfo.yres + guage_height) / 2; y++ ) {
            ...
            if ( vinfo.bits_per_pixel == 32 ) {
                ...
                *(fbp + location + 3) = 0;      // No transparency
                ...
                unsigned short int t = rgb16(r, g, b);
